...
class BayesDecoder:
    ...
    def fit_control(self, class_data: LoadData, PF_summary: pd.DataFrame, env='control_day1', train_ratio=0.8):
        df = []

        cells = PF_summary.loc[(PF_summary['env'] == env) & (PF_summary['mouse']==class_data.name)]['cell'].unique()
        env_laps = class_data.params['env_laps'][env]

        data = class_data.mean_activity[:, :, cells]
        n_train_lap = int(np.round(len(env_laps) * train_ratio, 0))
        train_y = np.tile(np.arange(40), n_train_lap)

        for n in range(self.nshuffle):
            selected_laps = np.random.choice(env_laps, n_train_lap, replace=False)
            test_laps = np.setdiff1d(env_laps, selected_laps, assume_unique=True)
            self.decoder.fit(np.vstack(data[selected_laps, :, :]), train_y)

            # calculate mean (min sq error) per lap
            test_y = self.decoder.predict(np.vstack(data[test_laps, :, :]))
            true_y = np.tile(np.arange(self.nbins), len(test_laps))
            error_y = np.abs(test_y - true_y)
            error_add = np.abs(test_y - true_y + self.nbins)
            error_min = np.abs(test_y - true_y - self.nbins)
            min_error_mat = np.minimum(error_y, np.minimum(error_add, error_min))
            ...
            #min_error_lap = np.reshape(min_error_mat, (40, -1), 'C')
            #mean_error_lap = np.mean(min_error_lap, axis=1)
            local = pd.DataFrame({'True': true_y, 'Pred': test_y, 'laps': test_laps, 'error': min_error_mat})
            local['shuffle'] = n
            local['mouse'] = class_data.name
            df.append(local)
