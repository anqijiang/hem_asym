...
class LoadRaw:
    def __init__(self, mouse, envs, day, remove_pauses=True, opto=True):
        self.name = mouse
        self.day = day
        self.env = envs
        ...
        if opto is True:
            ...
            file = load_py_var_mat(self.path, 'downsample-control_align_cell_mean.mat', ['Fc3_DF'])
            self.mat = file['Fc3_DF']
            self.beh = self.beh_params_downsample_control()

        self.constants = {'nframes': self.mat.shape[0], 'ncells': self.mat.shape[1]}
        self.logger = init_logger(self.path, self.name, 'assembly')

        if remove_pauses:
            pause_ind = find_pauses(self.beh['y'].to_numpy(), self.beh['velocity'].to_numpy())
            self.update_param_frames(pause_ind)

    def beh_params_downsample_control(self):
        beh_file = load_py_var_mat(self.path, 'downsample-control.mat')
        ...
        assert self.mat.shape[0] == len(ybinned), (f'{self.name} behavior and imaging of different size: '
                                                   f'imaging {self.mat.shape[0]} and behavior {len(ybinned)}')

        file = load_py_var_mat(self.path, 'downsample-control_align_cell_mean.mat', ['E'])
        E = file['E'].transpose()

        df = pd.DataFrame.from_dict(
            data={'y': ybinned[:, 0], 'velocity': velocity[:, 0], 'rewards': rewards[:, 0], 'lap': E[:, 0]})
        df['lap'] = df['lap'].replace(0, np.nan)
        df['lap'] = df['lap'].fillna(method='bfill')
        df['lap'] = df['lap'].fillna(value=df['lap'].max() + 1)
        df['lap'] = df['lap'].astype('int')
        df['env'] = 'downsampled_control_day1'
    ...
    def beh_params(self):
        ...
        # beh_file = load_py_var_mat(self.path, 'all-cond.mat')
        beh_file = load_py_var_mat(self.path, 'beh.mat')
        ybinned = beh_file['behavior']['ybinned'][0][0].transpose()
        velocity = beh_file['behavior']['velocity'][0][0].transpose()
        rewards = beh_file['behavior']['reward'][0][0].transpose()
        assert self.mat.shape[0] == len(ybinned), f'{self.name} behavior and imaging of different size'
        ...
        df['env'] = None
        df['opto'] = 'off'
        if 'switch_frame' in self.params:
    ...
    def update_param_frames(self, delete_frames: np.ndarray):
        ...
        self.logger.info(f'{self.name} deleting from {delete_frames[0]} to {delete_frames[1]}')
...
def bin_beh_mat(df, groupby_fun='mean', binwindow: int=5):
...
def plot_raster(data: LoadRaw, z=None, z_thresh=0, z_max=2, binwindow=10):
...
class AssemblyPipeline:
    ...
    def find_assembly_weights(self, data: LoadRaw):
        ...
        for env, mat in env_mat.items():
            save_path = os.path.join(data.path, f'{data.name}_{env}_assembly_weights_binwindow{self.binwindow}.npy')
            print(mat.shape)
            z = bin_z_mat(pd.DataFrame(mat), binwindow=self.binwindow)
            n_sig_assembly = n_assembly(z)
            weights, scores = find_assembly(z, n_sig_assembly)
            np.save(save_path, weights)
            assemb_strength = calculate_assembly_strength(weights, z)
            np.save(os.path.join(data.path, f'{data.name}_{env}_assembly_strength_binwindow{self.binwindow}.npy'), assemb_strength)
        ...
        strength_df.to_parquet(os.path.join(data.path, f'{data.name}_assembly_strength_binwindow{self.binwindow}.parquet'))

        return strength_df

    def get_assembly_summary(self, data: LoadRaw):

        weight_files = AssemblyPipeline.find_files(data, f'assembly_weights_binwindow{self.binwindow}.npy')
        strength_files = AssemblyPipeline.find_files(data, f'assembly_strength_binwindow{self.binwindow}.npy')
        envs = [re.search(r'e[LR]\d{2,3}_(.*?)_assembly_weights_binwindow\d+\.npy', filename).group(1) for filename in weight_files]
        cell_assembly = {}
        weights_all = []
        ...
        for n in range(len(weight_files)):
            ...
            weights_all.append(assembly_weights)
