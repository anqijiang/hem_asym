...
def rise_to_peak_opto(mat, prominence_thresh=0.1, amplitude=0.12, interval=10, width_min=5, width_max=120):
    smoothed = pd.DataFrame(mat).rolling(window=10, min_periods=3, center=True).mean().to_numpy().transpose()
    ...
    for i, row in enumerate(smoothed):
        ...
        for n in range(len(peaks)):
            ...
            transient_amplitude = np.nansum(result[i, left_idx:peaks[n] + 1])
            transient_amp.append([i, peaks[n], transient_amplitude])

    return pd.DataFrame(transient_amp, columns=['cell', 'frame', 'transient amplitude']), result, smoothed


def rise_to_peak(data: LoadAxon, prominence_thresh=0.05, amplitude=0.1, interval=10, width_min=5, width_max=100, moving_ave_size=10):

    print(f'{data.axon_df.isna().any(axis=1).sum()} total frames missing in {data.name} {data.day}')

    # add moving average filter
    mat = data.axon_df[data.axon_df.columns[1:-5]].rolling(window=moving_ave_size, min_periods=1, center=True).mean().to_numpy().transpose()

    #mat = data.axon_df.iloc[:, 1:-5].to_numpy().transpose()
    result = np.zeros_like(mat)
    for i, row in enumerate(mat):
        #smoothed = uniform_filter1d(row, size=moving_ave_size, mode='constant')
        peaks, properties = find_peaks(row, prominence=prominence_thresh, height=amplitude, distance=interval,
                                       width=[width_min, width_max])
...
def transient_summary_by_lap(data: LoadAxon):
    ...
    beh_df = data.axon_df[['lap', 'env', 'mouse', 'day', 'ybinned']].reset_index()
        ...
        for col_idx in range(r2p.shape[1]):
            ...
            if value != 0:  # If current value is non-zero
                ...
                if start_col is not None:
                    ...
                    data.append([row_idx, start_col, col_idx, group_sum])
    ...
    df = pd.DataFrame(data, columns=['cell', 'frame', 'peak', 'transient'])
    df = df.merge(beh_df, left_on='peak', right_on='index', how='left')
    df = df.drop(columns='index')
    df = df.sort_values(by=['cell', 'peak'])
